...
int main(int argc, char **argv)
{
    ...
    unsigned int vertexPosition = buffers[0];
    unsigned int texPosition = buffers[1];
    ...
    // glBindBuffer(GL_ARRAY_BUFFER, texPosition);
    // glBufferData(GL_ARRAY_BUFFER, 6 * sizeof(float), positions, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // unsigned int *blockBuffer = (unsigned int *)malloc(sizeof(blockSize));

        // std::memcpy(blockBuffer + offsets[0], innerColor, 4 * sizeof(float));
        // std::memcpy(blockBuffer + offsets[1], outerColor, 4 * sizeof(float));
        // std::memcpy(blockBuffer + offsets[2], &innerRadius, 4 * sizeof(float));
        // std::memcpy(blockBuffer + offsets[3], &outerRadius, 4 * sizeof(float));
        ...
        // mat4 rotationMatrix = glm::rotate(mat4(1.0f), 40.0f, vec3(0.0, 1.0, 0.0));
